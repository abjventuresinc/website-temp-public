...
import { FaFacebook, FaInstagram, FaLinkedin, FaPhone, FaEnvelope, FaMapMarkerAlt, FaPinterest, FaYelp } from "react-icons/fa";
...
const socialIconMap: Record<string, React.ReactElement> = {
  ...
  twitter: <FaSquareXTwitter className="w-5 h-5" />,
...
const defaultSocialLinks = Object.entries(ACTIVE_SOCIAL_MEDIA)
  .map(([key, href]) => ({
    icon: socialIconMap[key] || <FaFacebook className="w-5 h-5" />,
    href: href as string,
...
const Footer: React.FC<FooterProps> = ({
  logo = {
    url: "/",
    src: "/assets/config/logo-removebg-preview.png",
    alt: businessName,
    title: businessName,
  },
          ...
          <div className="flex w-full flex-col justify-between gap-6 lg:items-start">
            ...
            <div className="flex flex-col gap-2 mt-4 text-gray-400">
              ...
              <Link
                ...
                href={`tel:${contactInfoFinal.phone}`}
              >
                <FaPhone className="size-5" />
                <span>{contactInfoFinal.phone}</span>
              </Link>
              <Link
                className="flex items-center gap-2 text-sm hover:text-primary transition-colors"
